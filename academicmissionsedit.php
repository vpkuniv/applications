...
$academicmissions_edit = NULL; // Initialize page object first

class cacademicmissions_edit extends cacademicmissions {
	...
	// Page object name
	var $PageObjName = 'academicmissions_edit';
	...
	function Page_Main() {
		...
		if ($this->ID->CurrentValue == "")
			$this->Page_Terminate("academicmissionslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// AttendanceOnly
			$this->AttendanceOnly->SetDbValueDef($rsnew, $this->AttendanceOnly->CurrentValue, NULL, $this->AttendanceOnly->ReadOnly);

			// PresentAPaper
			$this->PresentAPaper->SetDbValueDef($rsnew, $this->PresentAPaper->CurrentValue, NULL, $this->PresentAPaper->ReadOnly);

			// Others
			$this->Others->SetDbValueDef($rsnew, $this->Others->CurrentValue, NULL, $this->Others->ReadOnly);

			// Participation
			$this->Participation->SetDbValueDef($rsnew, $this->Participation->CurrentValue, NULL, $this->Participation->ReadOnly);

			// Summary
			$this->Summary->SetDbValueDef($rsnew, $this->Summary->CurrentValue, NULL, $this->Summary->ReadOnly);

			// SuggestionRecommendation
			$this->SuggestionRecommendation->SetDbValueDef($rsnew, $this->SuggestionRecommendation->CurrentValue, NULL, $this->SuggestionRecommendation->ReadOnly);

			// FacultyMemberSign
			$this->FacultyMemberSign->SetDbValueDef($rsnew, $this->FacultyMemberSign->CurrentValue, NULL, $this->FacultyMemberSign->ReadOnly);

			// DepChairmanSign
			$this->DepChairmanSign->SetDbValueDef($rsnew, $this->DepChairmanSign->CurrentValue, NULL, $this->DepChairmanSign->ReadOnly);

			// DeanSign
			$this->DeanSign->SetDbValueDef($rsnew, $this->DeanSign->CurrentValue, NULL, $this->DeanSign->ReadOnly);
...
// Create page object
if (!isset($academicmissions_edit)) $academicmissions_edit = new cacademicmissions_edit();

// Page init
$academicmissions_edit->Page_Init();

// Page main
$academicmissions_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$academicmissions_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var academicmissions_edit = new ew_Page("academicmissions_edit");
academicmissions_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = academicmissions_edit.PageID; // For backward compatibility

// Form object
var facademicmissionsedit = new ew_Form("facademicmissionsedit");

// Validate form
facademicmissionsedit.Validate = function() {
...
facademicmissionsedit.Form_CustomValidate = 
...
facademicmissionsedit.ValidateRequired = true;
<?php } else { ?>
facademicmissionsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
facademicmissionsedit.Lists["x_College"] = {"LinkField":"x_CollegeID","Ajax":null,"AutoFill":false,"DisplayFields":["x_College_Name_EN","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
facademicmissionsedit.Lists["x_Department"] = {"LinkField":"x_DID","Ajax":null,"AutoFill":false,"DisplayFields":["x_DepartmentName","","",""],"ParentFields":["x_College"],"FilterFields":["x_CID"],"Options":[]};
...
<?php $academicmissions_edit->ShowPageHeader(); ?>
<?php
$academicmissions_edit->ShowMessage();
?>
<form name="facademicmissionsedit" id="facademicmissionsedit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="academicmissions">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_academicmissionsedit" class="table table-bordered table-striped">
...
<input type="hidden" data-field="x_ID" name="x_ID" id="x_ID" value="<?php echo ew_HtmlEncode($academicmissions->ID->CurrentValue) ?>">
<?php echo $academicmissions->ID->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
facademicmissionsedit.Lists["x_College"].Options = <?php echo (is_array($academicmissions->College->EditValue)) ? ew_ArrayToJson($academicmissions->College->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
facademicmissionsedit.Lists["x_Department"].Options = <?php echo (is_array($academicmissions->Department->EditValue)) ? ew_ArrayToJson($academicmissions->Department->EditValue, 1) : "[]" ?>;
</script>
</span>
...
ew_CreateCalendar("facademicmissionsedit", "x_StartDate", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
ew_CreateCalendar("facademicmissionsedit", "x_EndDate", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
facademicmissionsedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$academicmissions_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$academicmissions_edit->Page_Terminate();
?>
