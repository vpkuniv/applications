...
class ccountries_add extends ccountries {
	...
	function Page_Init() {
		...
		if (!$Security->CanAdd()) {
			...
			$this->Page_Terminate("countrieslist.php");
		}
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["NID"] != "") {
				$this->NID->setQueryStringValue($_GET["NID"]);
				$this->setKey("NID", $this->NID->CurrentValue); // Set up key
			} else {
				$this->setKey("NID", ""); // Clear key
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("NID")) <> "")
			$this->NID->CurrentValue = $this->getKey("NID"); // NID
		else
			$bValidKey = FALSE;
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->Nationality->FldIsDetailKey && !is_null($this->Nationality->FormValue) && $this->Nationality->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->Nationality->FldCaption());
		}
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$this->NID->setDbValue($conn->Insert_ID());
			$rsnew['NID'] = $this->NID->DbValue;
		}
		if ($AddRow) {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($countries->Nationality->FldCaption()) ?>");
...
<?php if ($countries->Nationality->Visible) { // Nationality ?>
	<tr id="r_Nationality">
		<td><span id="elh_countries_Nationality"><?php echo $countries->Nationality->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $countries->Nationality->CellAttributes() ?>>
<span id="el_countries_Nationality" class="control-group">
<textarea data-field="x_Nationality" name="x_Nationality" id="x_Nationality" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($countries->Nationality->PlaceHolder) ?>"<?php echo $countries->Nationality->EditAttributes() ?>><?php echo $countries->Nationality->EditValue ?></textarea>
</span>
<?php echo $countries->Nationality->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
